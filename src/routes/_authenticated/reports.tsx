...
function ReportsPage() {
  ...
  const { data: entries = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("stock_entries")
        .select(
          "id, shipping_cost, payment_method, restock_date, entry_type, stock_movements(quantity, initial_price)",
        )
  ...
  const partnerTxs = useMemo(
    () => (txs as any[]).filter((t) => t.sale_category === "partner"),
    [txs],
  );
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        min="0"
                        value={
                          report?.initial_cash != null ? report.initial_cash : initialCashInput
                        }
                ...
                >
                  <div className="space-y-1.5 max-w-xs">
                    ...
                    <Input
                      type="number"
                      min="0"
                      value={report?.initial_cash != null ? report.initial_cash : initialCashInput}
                      onChange={(e) => setInitialCashInput(e.target.value)}
                      placeholder="0"
                      disabled={report?.initial_cash != null || save.isPending}
                    />
