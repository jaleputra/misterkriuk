...
10. **Perbaikan Simpan Pengeluaran Gudang & Sinkronisasi Dashboard**:
    - **Perbaikan Constraint Database (Hotfix)**: Menambahkan file migrasi database baru [20260701000001_fix_gudang_category_constraint.sql](file:///Users/roughtell/amichicken/supabase/migrations/20260701000001_fix_gudang_category_constraint.sql) yang secara dinamis menghapus check constraint kategori lama pada tabel `products` dan membuat ulang constraint yang mengizinkan kategori `"gudang"` serta kategori soft-delete (`deleted_%`). Ini menjamin data belanjaan gudang dapat disimpan dengan sempurna tanpa error constraint database.
