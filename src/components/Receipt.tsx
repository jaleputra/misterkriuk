    ...
    <div id="receipt-print" className="font-mono text-xs bg-white text-black p-3 rounded-md border">
      ...
      {tx.items.map((i, idx) => (
        <div key={idx} className="mb-1">
          ...
          <div className="row flex justify-between"><span>{i.quantity} x {rupiah(i.price)}</span><span>{rupiah(i.subtotal)}</span></div>
        </div>
      ))}
      <hr className="border-t border-dashed border-black my-2" />
      <div className="row flex justify-between font-bold"><span>TOTAL</span><span>{rupiah(tx.total)}</span></div>
      <div className="row flex justify-between"><span>Bayar</span><span>{tx.payment_method.toUpperCase()}</span></div>
      {tx.payment_method === "cash" && (
        <>
          <div className="row flex justify-between"><span>Tunai</span><span>{rupiah(tx.cash_received ?? 0)}</span></div>
          <div className="row flex justify-between"><span>Kembalian</span><span>{rupiah(tx.change_amount ?? 0)}</span></div>
        </>
      )}
      <hr className="border-t border-dashed border-black my-2" />
